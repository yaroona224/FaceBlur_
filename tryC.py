# app.py — CardBlur (Render-safe, non-blocking model init, Safari/HTTPS webcam fix)
...
class ImageProcessor:
    def __init__(self):
        ...
        # Optional multi-language OCR (version-safe init)
        self.ocrs = []
        if USE_OCR:
    ...
    def run_live_doc_only(self, bgr_image: np.ndarray) -> np.ndarray:
        """
        Live stream behavior: blur WHOLE DOC only when a document exists AND it contains
        both a face and text. (Anti-flicker handled by client fps)
        """
        rgb = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2RGB)
...
# ---- Non-blocking lazy model init (so requests don't hang while weights load) ----
_uploader = None
_init_error = None
_init_started = False
...
def _init_model():
    global _uploader, _init_error
    try:
        print(f"[INFO] Initializing model from: {WEIGHTS_PATH}")
        _uploader = ImageProcessor()
        _init_error = None
        print("[INFO] Model ready.")
    except Exception as e:
        _init_error = str(e)
        _uploader = None
        print("[ERROR] Model init failed:", e, file=sys.stderr)

def get_uploader():
    global _uploader, _init_started
    if _uploader is not None or _init_error is not None:
        return _uploader
    with _init_lock:
        if not _init_started:
            _init_started = True
            threading.Thread(target=_init_model, daemon=True).start()
    # return None while it warms up; routes can fall back to original frame
    return None

_upload_jpeg = None  # stores last processed upload for download
...
<body>
  ...
  <!-- Hidden video element (Safari/HTTPS fix) -->
  ...
  <script>
        ...
        if (now - lastSent >= minInterval && videoEl.videoWidth){
          ...
          try{
            ...
            if (r.ok){
              ...
              // show unblurred frame if server not ready yet
              streamImg.src = off.toDataURL('image/jpeg', 0.8);
            }
          }catch(_e){
            streamImg.src = off.toDataURL('image/jpeg', 0.8);
          }
    ...
    document.getElementById('startCam').onclick = async ()=>{
      ...
      catch(e){ alert('Could not access camera. Use HTTPS and allow camera permission.'); }
    };
    document.getElementById('stopCam').onclick = ()=> stopClientCamera();

    window.addEventListener('keydown', (e)=>{
      if (e.key.toLowerCase() === 'r' && streamImg.src) streamImg.src = streamImg.src;
    });
    ...
    document.querySelectorAll('.options .btn').forEach(btn=>{
      ...
      btn.addEventListener('click', async ()=>{
        ...
        if (res.ok){
          ...
          alert('Processing failed: ' + txt);
        }
      });
    });

    downloadBtn.addEventListener('click', ()=>{
      if (!downloadBtn.disabled){ window.location.href = '/download_result?ts=' + Date.now(); }
    });

    preview.addEventListener('click', ()=>{ if (preview.src) window.open('/upload_result?ts=' + Date.now(), '_blank'); });
  </script>
</body>
</html>
"""
...
# -------- Camera start/stop (compat; no server-side webcam) --------
...
# -------- Live frame endpoint (browser → server blur → jpeg back) --------
...
def process_frame():
    ...
    up = get_uploader()
    if up is None:
        # Model not ready — return original frame so the page still works (no 5xx).
        ok, buf = cv2.imencode(".jpg", bgr, [int(cv2.IMWRITE_JPEG_QUALITY), 85])
        return Response(buf.tobytes(), mimetype="image/jpeg") if ok else ("Encode error", 500)

    out = up.run_live_doc_only(bgr)
...
def upload():
    global _upload_jpeg, _init_error
    ...
    up = get_uploader()
    if up is None:
        msg = f"Model not initialized: {_init_error or 'missing weights'}"
        return (msg, 503)

    mode = request.form.get("mode", "both")
    out = up.run_upload(bgr, mode=mode)
...
def download_result():
    ...
    return send_file(io.BytesIO(_upload_jpeg), mimetype="image/jpeg", as_attachment=True, download_name="cardblur_result.jpg")
...
if __name__ == "__main__":
    PORT = int(os.environ.get("PORT", 10000))  # Render uses $PORT; default 10000 if local
