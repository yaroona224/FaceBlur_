# tryS.py — CardBlur (Render-safe, lazy model init, no OCR)
...
# Text post-process (geometry & merging — still needed without OCR)
...
# =========================
# Upload processor — YOLO-only (no OCR), conservative text merge
# =========================
class ImageProcessor:
    ...
    def run_live_doc_only(self, bgr_image: np.ndarray) -> np.ndarray:
        """
        Live stream behavior: blur WHOLE DOC only when a document
        exists AND it contains both a face and text.
        """
        rgb = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2RGB)
...
# Lazy model init to avoid 502 if weights are missing
_uploader = None
  ...
  <script>
    ...
    document.getElementById('startCam').onclick = async ()=>{
      try{
        ...
        alert('Could not access camera. On public links, use HTTPS (e.g., Render automatically uses HTTPS).');
      }
    };
